<template>
  <v-row>
    <!-- 涨幅榜 -->
    <v-col cols="12" md="6">
      <v-sheet border class="h-100 ranking-sheet" rounded>
        <div class="ranking-header rise-header">
          <h3 class="text-subtitle-1 font-weight-medium">
            <v-icon class="mr-1" icon="mdi-trending-up" />
            涨幅榜 TOP10
          </h3>
        </div>
        <v-table class="ranking-table" density="compact">
          <thead>
            <tr>
          ...
          <tbody>
            <tr v-for="(item, index) in riseRanking" :key="item.code" class="rise-row">
              <td class="py-2">
                <v-chip class="rank-chip" :color="getRiseColor(item.changePercent)" size="small" variant="tonal">
                  {{ index + 1 }}
                </v-chip>
              </td>
              <td class="py-2">
              ...
              <td class="text-right">
                <v-chip class="percent-chip" :color="getRiseColor(item.changePercent)" size="small" variant="tonal">
                  <v-icon :icon="item.changePercent >= 0 ? 'mdi-trending-up' : 'mdi-trending-down'" size="x-small" start />
    ...
    <v-col cols="12" md="6">
      <v-sheet border class="h-100 ranking-sheet" rounded>
        <div class="ranking-header fall-header">
          <h3 class="text-subtitle-1 font-weight-medium">
            <v-icon class="mr-1" icon="mdi-trending-down" />
            跌幅榜 TOP10
          </h3>
        </div>
        <v-table class="ranking-table" density="compact">
          ...
          <tbody>
            <tr v-for="(item, index) in fallRanking" :key="item.code" class="fall-row">
              <td class="py-2">
                <v-chip class="rank-chip" :color="getFallColor(item.changePercent)" size="small" variant="tonal">
                  {{ index + 1 }}
                </v-chip>
              </td>
              <td class="py-2">
              ...
              <td class="text-right font-weight-medium">{{ item.price.toFixed(2) }}</td>
              <td class="text-right">
                <v-chip class="percent-chip" :color="getFallColor(item.changePercent)" size="small" variant="tonal">
                  <v-icon :icon="'mdi-trending-down'" size="x-small" start />
                  {{ item.changePercent.toFixed(2) }}%
                </v-chip>
              </td>
            </tr>
...
<script setup lang="ts">
  ...
  const allStocks = ref<StockItem[]>([
    ...
    { code: '600519', name: '贵州茅台', price: 1680, change: 152, changePercent: 9.95 },
    ...
    { code: '601318', name: '中国平安', price: 48.9, change: 3.12, changePercent: 6.82 },
    { code: '000333', name: '美的集团', price: 62.34, change: 3.89, changePercent: 6.65 },
    { code: '300750', name: '宁德时代', price: 178.9, change: 10.23, changePercent: 6.07 },
    ...
    { code: '600010', name: '包钢股份', price: 1.89, change: -0.18, changePercent: -8.7 },
    ...
    { code: '601398', name: '工商银行', price: 4.56, change: -0.25, changePercent: -5.2 },
